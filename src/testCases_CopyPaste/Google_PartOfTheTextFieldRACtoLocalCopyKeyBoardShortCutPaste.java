...
public class Google_PartOfTheTextFieldRACtoLocalCopyKeyBoardShortCutPaste extends testCases.Base {
	
	
	private WebDriver driver;
	private String currentSitePath;
	ExtentTest parentTest = ReportGenerator.initializeParentTest(getClass().getName().substring(10),"Testing Part of the Text Field from RAC to Local Copy Paste for Google using KeyBoard shortCut");
	...
	public void executeScript(){
		...
		try{
			...
			SikuliUtil.verifyObjectAndHighlight("TextInSearchbox", screen, parentTest, "Text get typed in searchbox", driver, testCaseName, "Yes");
			...
			//To select text			
			SikuliUtil.selectTextFromRAC(screen,400,0);
			...
			//Pasting the text in Excel
				CopyPasteFunctions.pasteClipboardTextandVerify(System.getProperty("user.dir")+"/CopyPasteTestData/TestData.xlsx", "PartOfTextFieldFromRACtoLocal", 1, 0, parentTest, "Write in Excel", testCaseName, this.driver);
				 
			
			Sleep(4000);
			//Final Site Run status
			ReportGenerator.logStatusPass(parentTest, testCaseName, "Testing Part of the Text Field from RAC to Local Copy Paste for Google using Keyboard shortcut testcase is working as expected");
			...
			ReportGenerator.logStatusFail(parentTest,testCaseName, "Testing Part of the Text Field from RAC to Local Copy Paste for Google using keyboard shortcut testcase Failed,Please see logs and error screenshots", this.driver);
		}
			finally{
