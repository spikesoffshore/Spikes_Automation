...
public class Google_AllTextFieldRACtoRACCopyKeyBoardShortCutPaste extends testCases.Base {
	
	
	private WebDriver driver;
	private String currentSitePath;
	ExtentTest parentTest = ReportGenerator.initializeParentTest(getClass().getName().substring(10),"Testing All Text Field from RAC to RAC Copy Paste for Google using KeyBoard shortCut");
	...
	public void executeScript(){
		...
		try{
			...
			if(flag==1){
				...
				SikuliUtil.verifyObjectAndClickOn("Yandex_searchbox", screen, parentTest, "Clicking yandex searchbox", driver, testCaseName, "No");
			...
			ReportGenerator.logStatusPass(parentTest, testCaseName, "Testing All Text Field from RAC to RAC Copy Paste for Google using Keyboard shortcut testcase is working as expected");
			...
			ReportGenerator.logStatusFail(parentTest,testCaseName, "Testing All Text Field from RAC to RAC Copy Paste for Google using keyboard shortcut testcase Failed,Please see logs and error screenshots", this.driver);
		}
			finally{
