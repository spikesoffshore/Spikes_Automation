...
public class Google_RACtoLocalCopyMouseShortCutPaste extends testCases.Base {
	
	private WebDriver driver;
	private String currentSitePath;
	ExtentTest parentTest = ReportGenerator.initializeParentTest(getClass().getName().substring(10),"Testing RAC to Local Copy Paste for Google using Mouse ShortCut");
	...
	public void executeScript(){
		...
		try{
			...
			SikuliUtil.verifyObjectAndType("SearchBox", screen, parentTest, "Google serach box", this.driver, testCaseName, "Yes", "cyberinc.com"+Key.ENTER);
			...
			SikuliUtil.clickLeft("RACTOLocal_CyberInc", screen, parentTest, "CLicking on left to the Search Result", driver, testCaseName, "No", 20);
			...
			SikuliUtil.mouseRightClick("RACTOLocal_CyberIncHighlighted", screen);
			Sleep(3000);
			...
			CopyPasteFunctions.pasteClipboardTextandVerify(System.getProperty("user.dir")+"/CopyPasteTestData/TestData.xlsx", "CopyFromExcelPasteInGoogleAndVi", 1, 1, parentTest, "Write in Excel", testCaseName, this.driver);
		...
		ReportGenerator.logStatusPass(parentTest, testCaseName, "RAC to Local Copy Paste for Google using Mouse Right Click testcase is working as expected");
		...
		ReportGenerator.logStatusFail(parentTest,testCaseName, "RAC to Local Copy Paste for Google using Mouse Right Click testcase Failed,Please see logs and error screenshots", this.driver);
	}
		finally{
