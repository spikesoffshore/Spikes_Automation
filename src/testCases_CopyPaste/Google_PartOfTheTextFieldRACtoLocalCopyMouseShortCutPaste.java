...
public class Google_PartOfTheTextFieldRACtoLocalCopyMouseShortCutPaste extends testCases.Base {
	
	
	private WebDriver driver;
	private String currentSitePath;
	ExtentTest parentTest = ReportGenerator.initializeParentTest(getClass().getName().substring(10),"Testing Part of the Text Field from RAC to Local Copy Paste for Google using Mouse shortCut");
	...
	public void executeScript(){
		...
		try{
			...
			//Pasting the text in Excel
				CopyPasteFunctions.pasteClipboardTextandVerify(System.getProperty("user.dir")+"/CopyPasteTestData/TestData.xlsx", "PartOfTextFieldFromRACtoLocal", 2, 1, parentTest, "Write in Excel", testCaseName, this.driver);
				 
			
			Sleep(4000);
			//Final Site Run status
			ReportGenerator.logStatusPass(parentTest, testCaseName, "Testing Part of the Text Field from RAC to Local Copy Paste for Google using Mouse shortcut testcase is working as expected");
			...
			ReportGenerator.logStatusFail(parentTest,testCaseName, "Testing Part of the Text Field from RAC to Local Copy Paste for Google using Mouse shortcut testcase Failed,Please see logs and error screenshots", this.driver);
		}
			finally{
